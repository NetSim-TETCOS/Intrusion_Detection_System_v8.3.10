			...
			default:
				{
					...
					default://Data packet or other layer control packet
						if(is_malicious(pstruEventDetails->nDeviceId,pstruEventDetails->dEventTime))
							Malicious_DropPacket(pstruEventDetails);
						else
...
#ifdef _NETSIM_MALICIOUS_
						if(is_malicious(pstruEventDetails->nDeviceId,pstruEventDetails->dEventTime))
							Malicious_AddRouteToCache(pstruEventDetails);
#endif
						DSR_PROCESS_RREQ();
						if(pstruEventDetails->pPacket && pstruEventDetails->pPacket->pstruNetworkData)
							pstruEventDetails->pPacket->pstruNetworkData->nTTL--;
